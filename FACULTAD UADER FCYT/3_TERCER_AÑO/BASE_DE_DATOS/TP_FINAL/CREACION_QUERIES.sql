...
-- ============================================================================
-- 5b POBLADO DE ROSTER
-- ============================================================================
-- Este bloque es la correccion, es necesario para vincular jugadores a equipos por temporada,
-- algo que faltaba en el script original y obligaba a las
-- consultas a usar la tabla staging_stats, cosa que nos corrigieron en la defensa.
INSERT INTO roster (jugador_id, equipo_id, temporada_id, numero_camiseta)
SELECT DISTINCT
    j.jugador_id,
    e.equipo_id,
    t.temporada_id,
    -- Toma el primer nro de camiseta valido
    MAX(CASE WHEN s.jerseyNo ~ '^[0-9]+$' THEN s.jerseyNo::smallint ELSE NULL END)
FROM staging_stats s
JOIN jugador j ON s.playerId = j.player_code
JOIN equipo e ON s.sigla = e.sigla
JOIN temporada t ON s.seasonId = t.descripcion
WHERE j.jugador_id IS NOT NULL
  AND e.equipo_id IS NOT NULL
  AND t.temporada_id IS NOT NULL
GROUP BY j.jugador_id, e.equipo_id, t.temporada_id
...
SELECT COUNT(DISTINCT r.jugador_id) AS cantidad_jugadores
FROM roster AS r
INNER JOIN equipo AS e ON r.equipo_id = e.equipo_id
...
WITH TeamReboundsPerGame AS (
    ...
    -- Unir con roster para saber el equipo (r.equipo_id) del jugador (jpe.jugador_id)
    -- en esa temporada (p.temporada_id)
    INNER JOIN roster AS r ON jpe.jugador_id = r.jugador_id AND p.temporada_id = r.temporada_id
    WHERE e_stat.codigo = 'REB'
      -- Asegurarse de que el equipo del roster sea uno de los que jugó el partido.
      -- Esto resuelve el jugador traspasado.
      AND r.equipo_id IN (p.equipo_local_id, p.equipo_visitante_id)
    GROUP BY r.equipo_id, jpe.partido_id
)
SELECT
    e.nombre,
...
INNER JOIN estadistica AS e_stat ON jpe.estadistica_id = e_stat.estadistica_id
INNER JOIN partido AS p ON jpe.partido_id = p.partido_id
-- Unir con roster para vincular el jugador a un equipo en esa temporada
INNER JOIN roster AS r ON jpe.jugador_id = r.jugador_id AND p.temporada_id = r.temporada_id
INNER JOIN equipo AS eq ON r.equipo_id = eq.equipo_id
...
WITH TeamAssistsPerGame AS (
    ...
    INNER JOIN estadistica AS e_stat ON jpe.estadistica_id = e_stat.estadistica_id
    INNER JOIN partido AS p ON jpe.partido_id = p.partido_id
    -- Unir con roster para saber el equipo (r.equipo_id) del jugador
    INNER JOIN roster AS r ON jpe.jugador_id = r.jugador_id AND p.temporada_id = r.temporada_id
    WHERE e_stat.codigo = 'AST'
      -- Asegurarse de que el equipo del roster sea uno de los que jugó el partido.
      AND r.equipo_id IN (p.equipo_local_id, p.equipo_visitante_id)
    GROUP BY r.equipo_id, jpe.partido_id
)
...
WHERE p.nombre NOT IN ('Usa', 'United States', 'Estados Unidos') -- Se agrega ESTODOS UNIDOS en español a la lista de excluidos
GROUP BY p.nombre
...
WITH PaisMasJugadores AS (
    ...
    FROM jugador AS j
    INNER JOIN pais AS p ON j.pais_id = p.pais_id
    WHERE p.nombre NOT IN ('Usa', 'United States', 'Estados Unidos')
    GROUP BY p.pais_id
...
WITH MattRyanGames AS (
    SELECT
        ...
        r.equipo_id AS ryan_team_id, -- equipo de matt ryan desde ROSTER
        p.equipo_ganador_id,
        ABS(p.puntos_local - p.puntos_visitante) AS diferencia
    FROM partido AS p
    INNER JOIN equipo AS e_local ON p.equipo_local_id = e_local.equipo_id
    INNER JOIN equipo AS e_visit ON p.equipo_visitante_id = e_visit.equipo_id
    -- Unimos con roster y jugador para encontrar los partidos de matt ryan
    INNER JOIN roster AS r ON p.temporada_id = r.temporada_id
    INNER JOIN jugador AS j ON r.jugador_id = j.jugador_id
    -- matt ryan haya jugadoen ese partido
    WHERE j.nombre = 'Matt' AND j.apellido = 'Ryan'
      AND EXISTS (
          SELECT 1 FROM jugador_partido_estad jpe
          WHERE jpe.partido_id = p.partido_id
            AND jpe.jugador_id = j.jugador_id
      )
)
SELECT
    partido_id,
    ...
    puntos_visitante
FROM MattRyanGames
WHERE ryan_team_id = equipo_ganador_id -- solo los partidos que gano
